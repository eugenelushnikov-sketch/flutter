<template>
	<div class="max-w-5xl mx-auto py-8 space-y-10">
		...
		<section class="space-y-3">
			<h2 class="font-semibold">Create Org</h2>
		...
		<section class="space-y-3">
			<h2 class="font-semibold">Create Unit</h2>
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
...
async function createOrg() {
	await $fetch(`${config.public.apiBase}/orgs`, { method: 'POST', body: org, credentials: 'include' });
	alert('Org created');
}
async function createProject() {
	await $fetch(`${config.public.apiBase}/projects`, { method: 'POST', body: project, credentials: 'include' });
	alert('Project created');
}
async function createUnit() {
	try {
		await $fetch(`${config.public.apiBase}/units`, { method: 'POST', body: { ...unit, project: { connect: { id: unit.projectId } } }, credentials: 'include' });
		alert('Unit created');
	} catch (e: any) {
